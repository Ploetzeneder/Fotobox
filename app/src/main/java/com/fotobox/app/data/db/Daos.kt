...
interface PhotoSessionDao {
    ...
    @Update suspend fun update(session: PhotoSession)
    @Delete suspend fun delete(session: PhotoSession)
...
interface AudioRecordingDao {
    ...
    @Delete suspend fun delete(recording: AudioRecording)
    @Query("SELECT * FROM audio_recordings ORDER BY createdAt DESC")
    fun getAll(): Flow<List<AudioRecording>>
