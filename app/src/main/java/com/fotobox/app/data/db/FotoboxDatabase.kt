package com.fotobox.app.data.db

import androidx.room.Database
import androidx.room.RoomDatabase
import androidx.room.TypeConverter
import androidx.room.TypeConverters
import com.fotobox.app.data.models.Photo
import com.fotobox.app.data.models.PhotoFilter
import com.fotobox.app.data.models.PhotoSession
import com.fotobox.app.data.models.StripLayout

class Converters {
    @TypeConverter
    fun fromStripLayout(value: StripLayout): String = value.name

    @TypeConverter
    fun toStripLayout(value: String): StripLayout = StripLayout.valueOf(value)

    @TypeConverter
    fun fromPhotoFilter(value: PhotoFilter): String = value.name

    @TypeConverter
    fun toPhotoFilter(value: String): PhotoFilter = PhotoFilter.valueOf(value)
}

@Database(
    entities = [PhotoSession::class, Photo::class],
    version = 1,
    exportSchema = false
)
@TypeConverters(Converters::class)
abstract class FotoboxDatabase : RoomDatabase() {
    abstract fun photoSessionDao(): PhotoSessionDao
    abstract fun photoDao(): PhotoDao
}
